...
class SlimDocumentTest < Minitest::Test
  ...
  def test_language_id_returns_erb
    document = create_document("h1 Hello")

    # Returns :erb so ruby-lsp's Definition listener applies the same
    # bare-method fallback it uses for ERB templates
    assert_equal :erb, document.language_id
  end
  ...
  def test_parse_recovers_from_errors
    ...
    # Should still have an AST (possibly empty), not crash
    refute_nil document.ast
  end
